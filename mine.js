...
var initSquare = function(row, col) {
    //初始化方格
    var contentHtml = `<div id="id-content"></div>`;
    var container = e(".container");
    container.insertAdjacentHTML("beforeend", contentHtml);

    var content = e("#id-content");
    for(let i = 0; i < row; i++) {
        //每一行
        var squareLineHtml = `<div class="square-line square-line-${i}"></div>`;
        content.insertAdjacentHTML("beforeend", squareLineHtml);

        var squareLine = e(`.square-line-${i}`);
        for(let j = 0; j < col; j++) {
            //每一小格子
            var squareHtml = `<div class="square" id=id-cell-${i}-${j}><span class="value"></span></div>`
            squareLine.insertAdjacentHTML("beforeend", squareHtml);
        }
    }
}
